...
namespace DCE_Manager.Utils
{
    ...
    public static class FormUtils
    {
        ...
        public static void CreateDCE_Folder()
        {
            ...
            //Main_Form.Instance.UpdateSharedData();
        ...
        private const long MaxLogSizeBytes = 500 * 1024; // 500 Ko (valeur de prod)
        //private const long MaxLogSizeBytes = 200 * 1024; // 200 Ko (test)
        //private const long MaxLogSizeBytes = 100 * 1024; // 100 Ko (test)
        //private const long MaxLogSizeBytes = 50 * 1024;  // 50 Ko (test rapide)
            ...
            lock (lockObj)
            {
                try
                {
                    ...
                    if (File.Exists(path))
                    {
                        ...
                        {
                            string backupPath = pathOptionInstaller + @"\log.old.txt";
                            try
                            {
                                // Un seul backup conservé : on écrase l'ancien s'il existe déjà
                                File.Copy(path, backupPath, overwrite: true);
                            }
                            catch
                            {
                                // Si la copie échoue (verrou, etc.), on ne bloque pas la purge
                            }
